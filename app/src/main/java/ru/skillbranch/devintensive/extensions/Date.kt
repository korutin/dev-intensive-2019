...
fun Date.humanizeDiff(date: Date = Date()): String {
    ...
    when (df) {
            ...
            in 75*60..22*60*60 -> r = when (df/3600L){
                                        in 5..19 -> "${df/3600L} часов назад"
            ...
            in 26*60*30..360*24*60*60 -> r = when (df/86400L) {
                in 5L..19L ->"${df/86400} дней назад"
                else -> when (df/86400L % 10) {
                    1L -> "${df / 86400} день назад"
                    in 2L..4L -> "${df / 86400} дня назад"
                    else -> "${df/86400} дней назад"
                }
            }
